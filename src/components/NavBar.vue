...
<template>
  ...
  >
    ...
    <!-- 工作日/周末筛选 -->
    <div class="flex items-center gap-4 bg-white/80 p-2 rounded-lg shadow-sm">
      <label class="inline-flex items-center cursor-pointer">
        <input 
          type="checkbox" 
          :checked="datasetStore.getShowWeekday"
          @change="datasetStore.setShowWeekday($event.target.checked)"
          class="sr-only peer" 
        />
        ...
        <span class="ml-2 text-sm font-medium text-gray-700">Weekday</span>
      </label>
      <label class="inline-flex items-center cursor-pointer">
        <input 
          type="checkbox" 
          :checked="datasetStore.getShowWeekend"
          @change="datasetStore.setShowWeekend($event.target.checked)"
          class="sr-only peer" 
        />
        <div class="relative w-11 h-6 bg-gray-200 peer-focus:outline-none peer-focus:ring-4 peer-focus:ring-purple-300 rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:border-gray-300 after:border after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600"></div>
